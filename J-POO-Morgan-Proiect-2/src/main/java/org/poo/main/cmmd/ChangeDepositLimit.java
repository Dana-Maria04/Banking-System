...
/**
 * The ChangeDepositLimit class is responsible for handling the command
 * to change the deposit limit of a business account. Only the owner of the
 * account can change the deposit limit.
 */
public class ChangeDepositLimit extends Command {

    /**
     * Constructs a ChangeDepositLimit command.
     *
     * @param users            The list of users in the system.
     * @param command          The command input containing details about the deposit limit change.
     ...
     * @param output           The output array node where the results of the
     *                         command will be stored.
     */
    public ChangeDepositLimit(final ArrayList<User> users, final CommandInput command,
    ...
    /**
     * Executes the ChangeDepositLimit command. It checks if the user is the owner of
     * the business account and updates the deposit limit if the user has the required privileges.
    ...
    public void execute() {
        ...
        for (BusinessAccount account : getBusinessAccounts()) {
            if (account.getOwnerEmail().equals(getCommand().getEmail())) {
                account.setDepositLimit(getCommand().getAmount());
                isOwner = true;
                break;
            }
        }
    ...
    /**
     * Provides functionality for undoing the command. Currently,
     * this method is reserved for future development.
     */
    @Override
    public void undo() {
    }
